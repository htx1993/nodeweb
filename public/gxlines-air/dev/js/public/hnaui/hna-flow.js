...
hnaui.define('jquery', function (exports) {
    ...
    var $ = hnaui.jquery, Flow = function (options) {
    }
        , ELEM_MORE = 'hnaui-flow-more'
        , ELEM_LOAD = '<i class="hnaui-anim hnaui-anim-rotate hnaui-anim-loop hnaui-icon ">&#xe63e;</i>';
    ...
    Flow.prototype.load = function (options) {
        ...
        var ELEM_TEXT = '<cite>加载更多</cite>'
            , more = $('<div class="hnaui-flow-more"><a href="javascript:;">' + ELEM_TEXT + '</a></div>');

        if (!elem.find('.hnaui-flow-more')[0]) {
            elem.append(more);
        }
    ...
    Flow.prototype.lazyimg = function (options) {
        ...
        var show = function (item, height) {
            ...
            if (elemTop >= start && elemTop <= end) {
                if (!item.attr('src')) {
                    var src = item.attr('hna-src');
                    hnaui.img(src, function () {
                        var next = that.lazyimg.elem.eq(index);
                        item.attr('src', src).removeAttr('hna-src');
