...
//异常提示
        error = function (msg) {
            win.console && console.error && console.error('hnaui hint: ' + msg);
        },
...
//内置模块
        modules = {
            hnaer: 'modules/hnaer' //弹层
            , hnadate: 'modules/hnadate' //日期
            , laypage: 'modules/hnapage' //分页
            , hnatpl: 'modules/hnatpl' //模板引擎
            , layim: 'modules/layim' //web通讯
            , layedit: 'modules/hnaedit' //富文本编辑器
            ...
            , 'hnaui.all': 'dest/hnaui.all' //PC模块合并版
        };
...
//定义模块
    Lay.fn.define = function (deps, callback) {
        ...
        if (hnaui['hnaui.all'] || (!hnaui['hnaui.all'] && hnaui['hnaui.mobile'])) {
...
//使用特定模块
    Lay.fn.use = function (apps, callback, exports) {
        ...
        if (apps.length === 0
            || (hnaui['hnaui.all'] && modules[item])
            || (!hnaui['hnaui.all'] && hnaui['hnaui.mobile'] && modules[item])
        ) {
        ...
        //回调
        function onCallback() {
            exports.push(hnaui[item]);
            apps.length > 1 ?
                that.use(apps.slice(1), callback, exports)
                : ( typeof callback === 'function' && callback.apply(hnaui, exports) );
        }
...
//css外部加载器
    Lay.fn.link = function (href, fn, cssname) {
        ...
        var id = link.id = 'hnauicss-' + app, timeout = 0;
...
//css内部加载器
    Lay.fn.addcss = function (firename, fn, cssname) {
        hnaui.link(config.dir + 'css/' + firename, fn, cssname);
    };
...
//执行自定义模块事件
    Lay.fn.event = function (modName, events, params) {
        ...
        hnaui.each(config.event[set], callback);
        filter[0] && hnaui.each(config.event[events], callback); //执行过滤器中的事件
        return result;
    };
