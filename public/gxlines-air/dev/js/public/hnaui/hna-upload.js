/*!
 ...
 @Title: hnaui.upload 单文件上传 - 全浏览器兼容版
 @Author: 贤心
 @License：MIT
...
hnaui.define('hnaer', function (exports) {
    ...
    var $ = hnaui.jquery;
    var layer = hnaui.layer;
    var device = hnaui.device();

    var elemDragEnter = 'hnaui-upload-enter';
    var elemIframe = 'hnaui-upload-iframe';
    ...
    Upload.prototype.init = function () {
        ...
        var body = $('body'), elem = $(options.elem || '.hnaui-upload-file');
        ...
        return elem.each(function (index, item) {
            ...
            var type = item.attr('hna-type') || options.type; //获取文件类型

            //包裹ui元素
            if (!options.unwrap) {
                form = '<div class="hnaui-box hnaui-upload-button">' + form + '<span class="hnaui-upload-icon"><i class="hnaui-icon">&#xe608;</i>' + (
                        item.attr('hna-title') || options.title || ('上传' + (fileType[type] || '图片') )
                    ) + '</span></div>';
            }
    ...
    Upload.prototype.action = function (input, type) {
        ...
        var item = $(input), ext = item.attr('hna-ext') || options.ext || ''; //获取支持上传的文件扩展名;
