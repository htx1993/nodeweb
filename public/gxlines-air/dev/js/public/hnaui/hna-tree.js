...
hnaui.define('jquery', function (exports) {
    ...
    var enterSkin = 'hnaui-tree-enter', Tree = function (options) {
    ...
    Tree.prototype.init = function (elem) {
        var that = this;
        elem.addClass('hnaui-box hnaui-tree'); //添加tree样式
        if (that.options.skin) {
            elem.addClass('hnaui-tree-skin-' + that.options.skin);
        }
        that.tree(elem);
        that.on(elem);
    };
    ...
    Tree.prototype.tree = function (elem, children) {
        ...
        hnaui.each(nodes, function (index, item) {
            var hasChild = item.children && item.children.length > 0;
            var ul = $('<ul class="' + (item.spread ? "hnaui-show" : "") + '"></ul>');
            var li = $(['<li ' + (item.spread ? 'data-spread="' + item.spread + '"' : '') + '>'
                //展开箭头
                , function () {
                    return hasChild ? '<i class="hnaui-icon hnaui-tree-spread">' + (
                ...
                , function () {
                    return options.check ? (
                        '<i class="hnaui-icon hnaui-tree-check">' + (
                ...
                , function () {
                    return '<a href="' + (item.href || 'javascript:;') + '" ' + (
                        ...
                        + ('<i class="hnaui-icon hnaui-tree-' + (hasChild ? "branch" : "leaf") + '">' + (
                            hasChild ? (
    ...
    Tree.prototype.spread = function (elem, item) {
        var that = this, options = that.options;
        var arrow = elem.children('.hnaui-tree-spread')
        ...
        var open = function () {
            if (elem.data('spread')) {
                ...
                ul.removeClass('hnaui-show');
                arrow.html(icon.arrow[0]);
                a.find('.hnaui-icon').html(icon.branch[0]);
            } else {
                elem.data('spread', true);
                ul.addClass('hnaui-show');
                arrow.html(icon.arrow[1]);
                a.find('.hnaui-icon').html(icon.branch[1]);
            }
        };
    ...
    Tree.prototype.on = function (elem) {
        ...
        if (options.drag) {
            $(document).on('mousemove', function (e) {
                var move = that.move;
                if (move.from) {
                    var to = move.to, treeMove = $('<div class="hnaui-box ' + dragStr + '"></div>');
                    e.preventDefault();
                    $('.' + dragStr)[0] || $('body').append(treeMove);
                    var dragElem = $('.' + dragStr)[0] ? $('.' + dragStr) : treeMove;
                    (dragElem).addClass('hnaui-show').html(move.from.elem.children('a').html());
                    dragElem.css({
                        left: e.pageX + 10
                        , top: e.pageY + 10
                    })
    ...
    exports('tree', function (options) {
        ...
        if (!elem[0]) {
            return hint.error('hnaui.tree 没有找到' + options.elem + '元素');
        }
        tree.init(elem);
    });
});
